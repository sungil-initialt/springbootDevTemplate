...
public class RequestMappingAnnotationRegister {
    ...
    public RequestMappingAnnotationRegister(ApplicationContext applicationContext) {
        synchronized (RequestMappingAnnotationRegister.class) {
            ...
            log.debug(LoggingUtil.makeFwLogForm("All Registered Request Mappings and Annotations", requestAnnotationRegister.toString(), CommonConstants.FW_RESOURCE_LOG_TAG));
            log.info(LoggingUtil.makeFwLogForm("Handlers with No URL Pattern (Potential Mapping Error)", logBodyForPathEmpty.isEmpty() ? "No Error (Good)" : logBodyForPathEmpty.toString(), CommonConstants.FW_RESOURCE_LOG_TAG));
            log.info(LoggingUtil.makeFwLogForm("Handlers with No Specific HTTP Method (Not Recommended)", logBodyForNonSpecificMapping.isEmpty() ? "All Handlers are mapped with Specific HTTP Method (Good)" : logBodyForNonSpecificMapping.toString(), CommonConstants.FW_RESOURCE_LOG_TAG));
        }
    }
